// function drawPin(x, y, pinAngle, intensity, distance = 200) {
//     ctx.save();
//     ctx.translate(x, y);
//     // 1. normalize for p5: p5 handles 0 degrees at +90deg from the web browser
//     ctx.rotate(radians(-90))
//     // 2. rotate accordingly
//     ctx.rotate(radians(pinAngle));
//     // line(0, 0, 50, 0);
//     // noFill();
//     // const colorTest = [0,0,0];
//     // fill(colorTest);
//     ctx.fillStyle(asignColor(intensity));

//     // stroke(asignColor(intensity));
//     // strokeWeight(20)
//     // console.log(distance);
//     // const distanceMap = map(distance, 0, 0.05, 0, width);
//     const distanceMap = mapRange(distance, 0, 500, 0, width);
//     // console.log(distance);
//     // const distanceMap = map(mouseX, 300,width, 0, width);
//     // console.log(distanceMap);
  
//     circle(distanceMap, 0, 700);
//     ctx.restore();
// }
function asignColor(statut) {
    let checkColor
    switch (statut) {
        case 'parc':
            checkColor = "green";
            break;
        case 'work':
            checkColor = "red";
            break;
        case 'lac':
            checkColor = "blue";
            break;
        default:
            console.log("bug");
    }
    return checkColor;
}
function calcCrow(lat1, lon1, lat2, lon2) {
    if ((lat1 == lat2) && (lon1 == lon2)) {
        return 0;
    }
    else {
        var radlat1 = Math.PI * lat1 / 180;
        var radlat2 = Math.PI * lat2 / 180;
        var theta = lon1 - lon2;
        var radtheta = Math.PI * theta / 180;
        var dist = Math.sin(radlat1) * Math.sin(radlat2) + Math.cos(radlat1) * Math.cos(radlat2) * Math.cos(radtheta);
        if (dist > 1) {
            dist = 1;
        }
        dist = Math.acos(dist);
        dist = dist * 180 / Math.PI;
        dist = dist * 60 * 1.1515;
        return dist;
    }
}
